...
function setDateDetails(date, isoString) {
  ...
  if (typeof Date.parse(isoString) === null) {
    return "Помилка: недійсний ISO рядок";
  }

  let newIsoDate = new Date(isoString);

  date.setTime(newIsoDate.getTime());

  return date;
  //   return date.setTime(isoString.getTime());
  // Перевірка, чи є вхідне значення об'єктом Date,це можно зробити перевіривши чи є date.getTime по типу функція .
  // Якщо date не є об'єктом Date, повертаємо рядок
  // "Помилка: вхідне значення має бути об'єктом Date"
...
function isLeapYear(year) {
  //   const nowDate = new Date(year);
  //   const years = nowDate.getFullYear();
  //   const days = nowDate.getDate();
  //   const isLeap = Date.parse(days) / 4;
  //   //   const isLeap = years / 4;
  //   if (typeof years === "number" || typeof isLeap === true) {
  //     return { year: years, isLeap: isLeap };
  // 	}

  const date = new Date();

  const isLeap = date.getDate();
  console.log(isLeap);

  if (typeof year === "number" || typeof isLeap % 4 === 0) {
    return {
      year,
      isLeap,
    };
...
function getDayOfWeek(date) {
  ...
  const dayOfWeek = daysOfWeek[weekNumber];
...
function getAge(birthDate) {
  ...
  const age = perevir ? yearDiff - 1 : yearDiff;
...
function compareDates(date1, date2) {
  if (
    !(
      typeof date1.getTime === "function" || typeof date2.getTime === "function"
    )
  ) {
    return "Помилка: вхідне значення має бути об'єктом Date";
  }
...
function getDaysDifference(startDate, endDate) {
  ...
  const startMili = startDate.getTime();
  const endMili = endDate.getTime();
  const timeDiff = endMili - startMili;
  const daysDifference = timeDiff / (1000 * 60 * 60 * 24);

  const startDateISO = startDate.toISOString();
  const endDateISO = endDate.toISOString();

  //   const daysDifference = a.toISOString();
